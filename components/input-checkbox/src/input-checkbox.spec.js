...
it('should call onChange handler on change', () => {
  ...
  const wrapper = mount(<InputCheckBox onChange={handler} />)
  wrapper.find('input').simulate('change', { target: { checked: true } })
  expect(handler).toHaveBeenCalled()
})

it('should ignore change events if no change handler', () => {
  const wrapper = mount(<InputCheckBox onChange={null} />)
  wrapper.find('input').simulate('change')
})

it('should render title component beside the checkbox', () => {
  const wrapper = mount(<InputCheckBox onChange={null} title="Text" />)
